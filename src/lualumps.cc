...
// Initialize the part of the wad package that deals with lumps.  Assumes that
// the 'wad' library table is at the top of the stack.
void luaopen_lumps(lua_State* L) {
	// Fetch functions we created in Lua
	Lua::doBuffer(L, reinterpret_cast<char*>(lualumps_lua), lualumps_lua_len, "luawad.lua (internal)");
	// [wadlib][wadfuncs][Lumpsfuncs]

	// Create "Lumps" userdata
	luaL_newmetatable(L, WADmake::META_LUMPS);
	// [wadlib][wadfuncs][Lumpsfuncs][Lumpsmeta]
	lua_pushvalue(L, -1);
	// [wadlib][wadfuncs][Lumpsfuncs][Lumpsmeta][Lumpsmeta]
